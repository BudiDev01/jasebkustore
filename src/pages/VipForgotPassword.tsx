...
const VipForgotPassword = () => {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: `${window.location.origin}/vip/forgot-password`,
    });
    setLoading(false);
    if (error) {
      toast({ title: "Error", description: error.message, variant: "destructive" });
    } else {
      toast({ title: "OTP Sent!", description: "Check your Gmail inbox for the recovery code." });
      setStep("otp");
    }
  };
  ...
  return (
    <div className="min-h-screen gradient-hero flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <Link to="/vip" className="flex items-center gap-2 justify-center mb-8">
          <div className="w-10 h-10 rounded-xl gradient-gold flex items-center justify-center shadow-gold">
            <Crown className="w-5 h-5 text-primary-foreground" />
          </div>
          <span className="font-bold text-xl text-white">
            <span className="text-gold">VIP</span> Recovery
          </span>
        </Link>
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 backdrop-blur-xl p-8 shadow-navy">
          ...
          <p className="text-white/50 text-sm mb-6">
            {step === "email" && "Enter your Gmail to receive a recovery code"}
          ...
          {step === "email" && (
            <form onSubmit={handleSendOtp} className="space-y-4">
              <div>
                <Label className="text-white/70 text-sm">Gmail Address</Label>
          ...
          <p className="text-center text-white/30 text-xs mt-6">
            <Link to="/vip" className="hover:text-white/50">← Back to VIP Login</Link>
          </p>
        </div>
      </div>
    </div>
...
export default VipForgotPassword;
